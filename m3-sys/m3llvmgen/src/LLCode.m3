...
; IMPORT LLTypes 
; FROM LLTypes 
    IMPORT Operand , MVar , Regno , OLoc , VLoc , LLVarTyp , LLProcTyp , NRegs 
; FROM LLTypes IMPORT OperandSize , GetOperandSize , RegistersForByteOperations 
; FROM LLTypes IMPORT RegName , SplitOperand , TypeIs64 , SplitImm , OperandPart 
; FROM LLTypes IMPORT GetTypeSize , TZero , TOne , EAX , EDX , ESP , EBP , ECX 
; IMPORT LLWr 
...
; REVEAL T 
    = Public BRANDED "LLCode.T" OBJECT 
      ...
      ; fstackloaded := 0 
      ; ftop_mem : MVar 
      ; labarr : REF ARRAY OF LLLabelTyp 
      ; lablimit := 0 
      ; next_label_id := 0 
      ; f_litlist : FLiteral := NIL 
      ; abscall_list : AbsCall := NIL 
      ; flitvar : LLVarTyp := NIL 
      ; n_tags : INTEGER := 0 
...
; PROCEDURE intCall ( t : T ; label : Label ) 
  ...
  ; BEGIN 
    ...
    ; WITH lab = t . labarr [ label ] , curs = t . obj . cursor ( Seg . Text ) 
      DO 
 
        ins . opcode := 16_E8 
      ; ins . dsize := 4 
      ; IF NOT lab . no_address 
        THEN 
      ...
      ; IF lab . no_address 
        THEN 
          log_unknown_label 
            ( t , label , t . obj . cursor ( Seg . Text ) - 4 , FALSE ) 
        END 
      END 
    END intCall 
...
; TYPE AbsCall 
    = REF RECORD sym : INTEGER ; loc : ByteOffset ; link : AbsCall ; END 
 
; PROCEDURE absCall ( t : T ; LLProc : LLProcTyp ) 
  = VAR ins : Instruction 
  ; BEGIN 
      ins . opcode := 16_FF 
    ; ins . modrm := 16_15 
  ...
  ; BEGIN 
    ...
    ; WITH lab = t . labarr [ label ] , curs = t . obj . cursor ( Seg . Text ) 
      ...
      ; IF ins . disp > 16_7F OR ins . disp < - 16_80 
           OR lab . no_address AND NOT lab . short 
        THEN 
          IF lab . no_address 
          THEN 
            ins . disp := 0 
      ...
      ; IF lab . no_address 
        THEN 
          ...
          THEN 
            log_unknown_label 
              ( t , label , t . obj . cursor ( Seg . Text ) - 1 , FALSE ) 
          ELSE 
            log_unknown_label 
              ( t , label , t . obj . cursor ( Seg . Text ) - 4 , FALSE ) 
          END 
...
; PROCEDURE prepare_stack ( t : T ; op : FOp ; forcenomem := FALSE ) 
  = BEGIN 
      WITH opc = fopcode [ op ] 
      DO 
        IF ( NOT opc . takesmem ) OR forcenomem 
        THEN 
          IF ( opc . stackin > 0 OR opc . stackdiff # 0 ) AND t . ftop_inmem 
          THEN 
            fstack_loadtop ( t ) 
          END 
        ; IF opc . stackdiff > 0 
    ...
    ; IF t . ftop_inmem 
      ...
      ; build_modrm 
          ...
          , Operand 
              { loc := OLoc . mem 
              , mvar := t . ftop_mem 
              , optype := t . ftop_mem . mvar_type 
              } 
          , t . opcode [ fopcode [ op ] . memop ] 
          , mem 
          ) 
      ; writecode ( t , mem ) 
...
; PROCEDURE memFOp ( t : T ; op : FOp ; mvar : MVar ) 
  ...
  ; BEGIN 
    ...
    ; build_modrm 
        ( t 
        , Operand 
            { loc := OLoc . mem , mvar := mvar , optype := mvar . mvar_type } 
        , t . opcode [ fopcode [ op ] . memop ] 
        , ins 
        ) 
...
; PROCEDURE shift_double_immediate 
    ( t : T 
    ; singleOp : Op 
    ; READONLY destA : ARRAY OperandPart OF Operand 
  ...
  ;   doubleOp 
      := VAL ( left * ORD ( Op . oSHLD ) + right * ORD ( Op . oSHRD ) , Op ) 
  ;   immMinus32 : TIntN . T 
  ; BEGIN 
      IF TIntN . GE ( imm , TIntN . ThirtyTwo ) 
      THEN 
        IF TIntN . NE ( imm , TIntN . ThirtyTwo ) 
        THEN 
      ...
      ELSE 
        ...
        shift_double_op 
          ...
          , destA [ left ] 
          , destA [ right ] 
          , Operand { loc := OLoc . imm , imm := imm } 
          ) 
      ; t . immOp ( singleOp , destA [ right ] , imm ) 
      END 
    END shift_double_immediate 
 
; PROCEDURE add_double_immediate 
    ( t : T 
    ; READONLY destA : ARRAY OperandPart OF Operand 
...
; PROCEDURE subtract_double_immediate 
    ( t : T 
    ; READONLY destA : ARRAY OperandPart OF Operand 
    ; immA : ARRAY OperandPart OF TIntN . T 
    ) 
...
; PROCEDURE subtract_double 
    ( t : T ; READONLY destA , srcA : ARRAY OperandPart OF Operand ) 
  = BEGIN 
      t . binOp ( Op . oSUB , destA [ 0 ] , srcA [ 0 ] ) 
    ; t . binOp ( Op . oSBB , destA [ 1 ] , srcA [ 1 ] ) 
    END subtract_double 
 
; PROCEDURE compare_double_immediate 
    ( t : T 
    ; READONLY destA : ARRAY OperandPart OF Operand 
    ; immA : ARRAY OperandPart OF TIntN . T 
    ) 
...
; PROCEDURE binOp_double 
    ( t : T ; op : Op ; READONLY destA , srcA : ARRAY OperandPart OF Operand ) 
  = 
(* oOR, oXOR, oAND *) 
    BEGIN 
...
; PROCEDURE unOp_double 
    ( t : T ; op : Op ; READONLY destA : ARRAY OperandPart OF Operand ) 
  = 
(* oNOT *) 
    BEGIN 
      t . unOp ( op , destA [ 0 ] ) 
...
; PROCEDURE shift_double_ecx 
    ( t : T ; singleOp : Op ; READONLY destA : ARRAY OperandPart OF Operand ) 
  = VAR right_signed := ( singleOp = Op . oSAR ) 
  ;   left := ORD ( singleOp = Op . oSHL ) 
  ;   right := ORD ( ( singleOp = Op . oSHR ) OR right_signed ) 
  ;   doubleOp 
  ...
  ; BEGIN 
      ...
      shift_double_op 
        ( t , doubleOp , destA [ left ] , destA [ right ] , t . reg [ ECX ] ) 
    ; t . unOp ( singleOp , destA [ right ] ) 
    ; testOp 
        ( t 
        , t . reg [ ECX ] 
        , Operand { loc := OLoc . imm , imm := TIntN . ThirtyTwo } 
        ) 
...
; PROCEDURE negate_double ( t : T ; READONLY destA : ARRAY OperandPart OF Operand ) 
  = BEGIN 
    ...
    ; t . binOp 
        ( Op . oADC 
        , destA [ 1 ] 
        , Operand { loc := OLoc . imm , imm := TZero , optype := Type . Word32 } 
        ) 
    ; t . unOp ( Op . oNEG , destA [ 1 ] ) 
    END negate_double 
 
; PROCEDURE shift_double_op 
    ( t : T ; op : Op ; READONLY dest , src , shiftCount : Operand ) 
  = 
...
<* ASSERT NOT TypeIs64(dest.optype) *> 
    ...
    ; IF shiftCount . loc = OLoc . imm 
      THEN 
        IF TWordN . GT ( shiftCount . imm , TWordN . Max8 ) 
        THEN 
          Err 
            ( t 
            , "shift_double_op: shift count must fit in a byte:" 
              & TIntN . ToDiagnosticText ( shiftCount . imm ) 
            ) 
        END 
      ; IF NOT TIntN . ToHostInteger ( shiftCount . imm , ins . imm ) 
        THEN 
          Err 
            ( t 
            , "shift_double_op: ToHostInteger(shiftCount.imm) failed:" 
              & TIntN . ToDiagnosticText ( shiftCount . imm ) 
            ) 
        END 
      ; ins . imsize := 1 
...
; PROCEDURE immOp1 
    ( t : T ; op : Op ; READONLY dest : Operand ; READONLY imm : TIntN . T ) 
  = VAR ins : Instruction 
  ;   immalt := TIntN . T { n := 4 * GetOperandSize ( dest ) , x := imm . x } 
  ; BEGIN 
                <* ASSERT dest.loc = OLoc.register OR dest.loc = OLoc.mem *> 
 
      IF NOT TIntN . ToHostInteger ( imm , ins . imm ) 
      THEN 
        Err 
          ( t 
          , "immOp1: unable to convert immediate to INTEGER:" 
            & TIntN . ToDiagnosticText ( imm ) 
          ) 
      END 
 
    ; IF TIntN . GE ( immalt , TIntN . Min8 ) 
         AND TIntN . LE ( immalt , TIntN . Max8 ) 
      THEN 
        ins . imsize := 1 
      ELSE 
        ins . imsize := 4 
    ...
    ; IF dest . loc = OLoc . register AND dest . reg [ 0 ] = EAX 
      ...
      ; IF ins . imsize = 1 
        THEN 
          IF dest . loc = OLoc . mem 
             AND CG_Bytes [ dest . mvar . mvar_type ] = 1 
          THEN 
            ins . opcode := opcode [ op ] . imm32 - 1 
          ; writecode ( t , ins ) 
          ; log_global_var ( t , dest . mvar , - 5 ) 
          ELSIF dest . loc = OLoc . mem 
             AND CG_Bytes [ dest . mvar . mvar_type ] = 2 
          THEN 
            ins . prefix := TRUE 
          ; ins . opcode := opcode [ op ] . imm8 
          ; writecode ( t , ins ) 
          ...
          ; 
            ...
            IF op IN SET OF Op { Op . oSHL , Op . oSHR , Op . oSAR } 
               AND TIntN . EQ ( imm , TIntN . One ) 
            THEN 
...
<* ASSERT NOT TypeIs64(destA[0].optype) *> 
      ...
      THEN 
        ...
        OF 
        | Op . oADD 
          =>  add_double_immediate ( t , destA , immA ) 
        | Op . oSUB 
          =>  subtract_double_immediate ( t , destA , immA ) 
        | Op . oCMP 
          =>  compare_double_immediate ( t , destA , immA ) 
        | Op . oSHL , Op . oSHR , Op . oSAR 
          =>  shift_double_immediate ( t , op , destA , imm ) 
        ELSE 
            <* ASSERT FALSE *> 
        END 
      ELSE 
...
<* ASSERT NOT TypeIs64(destA[0].optype) *> 
      ...
      THEN 
        Err 
          ( t 
          , "binOp: size mismatch: destSize:" & Fmt . Int ( destSize ) 
            & " srcSize:" 
            & Fmt . Int ( srcSize ) 
            & " src.optype:" 
            & Target . TypeNames [ src . optype ] 
            & " dest.optype:" 
            & Target . TypeNames [ dest . optype ] 
          ) 
      END 
    ...
    ; IF srcSize = 2 
      THEN 
        ...
        OF 
        | Op . oADD 
          =>  add_double ( t , destA , srcA ) 
        | Op . oSUB 
          =>  subtract_double ( t , destA , srcA ) 
        | Op . oCMP 
          =>  compare_double ( t , destA , srcA ) 
        | Op . oOR , Op . oXOR , Op . oAND 
          =>  binOp_double ( t , op , destA , srcA ) 
        ELSE 
          <* ASSERT FALSE *> 
        END 
      ELSE 
...
; PROCEDURE tableOp 
  ...
  ; BEGIN 
    ...
    ; ins . mrm_present := TRUE 
    ; IF ( NOT fully_known ) OR ( ins . disp > 16_7F ) 
         OR ( ins . disp < - 16_80 ) 
      THEN 
        ins . dsize := 4 
...
; PROCEDURE swapOp1 ( t : T ; READONLY dest , src : Operand ) 
  ...
  ; BEGIN 
      ...
      IF dest . loc = OLoc . register AND src . loc = OLoc . register 
         AND ( dest . reg [ 0 ] = EAX OR src . reg [ 0 ] = EAX ) 
      THEN 
        IF dest . reg [ 0 ] = EAX 
        THEN 
          otherreg := src . reg [ 0 ] 
...
; PROCEDURE swapOp ( t : T ; READONLY dest , src : Operand ) 
  ...
  ; BEGIN 
      ...
      THEN 
        Err 
          ( t 
          , "swapOp: size mismatch: destSize:" & Fmt . Int ( destSize ) 
            & " srcSize:" 
            & Fmt . Int ( srcSize ) 
            & " src.optype:" 
            & Target . TypeNames [ src . optype ] 
...
; CONST MOVSW = Instruction { prefix := TRUE , opcode := 16_A5 } 
...
; PROCEDURE movOp1 ( t : T ; READONLY dest , src : Operand ) 
  ...
  ; BEGIN 
    ...
    ; IF dest . loc = OLoc . register AND dest . reg [ 0 ] = EAX 
         AND src . loc = OLoc . mem 
         AND CG_Bytes [ src . mvar . mvar_type ] = 4 
         AND src . mvar . var . loc = VLoc . global 
      THEN 
        Mn ( t , "MOV" ) 
      ; MnOp ( t , dest ) 
      ; MnOp ( t , src ) 
    ...
    ; IF src . loc = OLoc . register AND src . reg [ 0 ] = EAX 
         AND dest . loc = OLoc . mem 
         AND dest . mvar . var . loc = VLoc . global 
      THEN 
        Mn ( t , "MOV" ) 
      ; MnOp ( t , dest ) 
      ; MnOp ( t , src ) 
    ...
    ; IF dest . loc = OLoc . register AND src . loc = OLoc . mem 
         AND CG_Bytes [ src . mvar . mvar_type ] < 4 
      THEN 
        CASE src . mvar . mvar_type 
        OF 
        | Type . Word8 
          =>  ins . opcode := 16_8A 
            ; mnemonic := "MOV" 
...
; PROCEDURE movOp ( t : T ; READONLY dest , src : Operand ) 
  ...
  ; BEGIN 
      ...
      THEN 
        Err 
          ( t 
          , "movOp: size mismatch: destSize:" & Fmt . Int ( destSize ) 
            & " srcSize:" 
            & Fmt . Int ( srcSize ) 
            & " src.optype:" 
            & Target . TypeNames [ src . optype ] 
...
; PROCEDURE movDummyReloc ( t : T ; READONLY dest : Operand ; sym : INTEGER ) 
  ...
  ; BEGIN 
    ...
    ; writecode ( t , ins ) 
    ; t . obj . relocate 
        ( t . textsym , t . obj . cursor ( Seg . Text ) - 4 , sym ) 
    END movDummyReloc 
...
; PROCEDURE movImmT ( t : T ; READONLY dest : Operand ; imm : TIntN . T ) 
  ...
  ; BEGIN 
      ...
      THEN 
        Err 
          ( t 
          , "movImmT: unable to convert immediate to INTEGER:" 
            & TIntN . ToDiagnosticText ( imm ) 
          ) 
      END 
    ; IF dest . loc # OLoc . register 
      ...
      ; writecode ( t , ins ) 
      ; log_global_var 
          ( t , dest . mvar , - 4 - CG_Bytes [ dest . mvar . mvar_type ] ) 
      ELSE 
      ...
      ; one 
          := ( NOT zero ) AND ( opsize = 1 ) AND TIntN . EQ ( imm , TOne ) 
      ; IF zero OR one 
        THEN 
        ...
        ELSIF TWordN . EQ 
             ( imm 
             , TIntN . T { n := 4 * GetOperandSize ( dest ) , x := TInt . MOne } 
             ) 
        THEN 
          immOp ( t , Op . oOR , dest , imm ) 
        ELSIF ( opsize = 1 ) AND ( dest . reg [ 0 ] # ESP ) 
           AND TIntN . GE ( imm , TIntN . Min8 ) 
           AND TIntN . LE ( imm , TIntN . Max8 ) 
        THEN 
          pushOp 
            ( t 
            , Operand 
                { loc := OLoc . imm , imm := imm , optype := dest . optype } 
            ) 
        ; popOp ( t , dest ) 
        ELSE 
          ins . opcode := 16_B8 + dest . reg [ 0 ] 
...
; PROCEDURE movImmI ( t : T ; READONLY dest : Operand ; imm : INTEGER ) 
  = VAR immT : TIntN . T 
  ; BEGIN 
      IF NOT TIntN . FromHostInteger ( imm , BYTESIZE ( imm ) , immT ) 
      THEN 
...
; PROCEDURE pushOp1 ( t : T ; READONLY src : Operand ) 
  ...
  ; BEGIN 
    ...
    ; CASE src . loc 
      OF 
      | OLoc . imm 
        =>  IF NOT TIntN . ToHostInteger ( src . imm , ins . imm ) 
            THEN 
              Err 
                ( t 
                , "pushOp: unable to convert immediate to INTEGER:" 
                  & TIntN . ToDiagnosticText ( src . imm ) 
                ) 
            END 
          ; IF TIntN . GE ( src . imm , TIntN . Min8 ) 
               AND TIntN . LE ( src . imm , TIntN . Max8 ) 
            THEN 
              ins . opcode := 16_6A 
            ; ins . imsize := 1 
            ELSE 
...
<* ASSERT bits.loc = OLoc.register *> 
      ...
      IF index . loc = OLoc . imm 
      THEN 
        IF NOT TIntN . ToHostInteger ( index . imm , ins . imm ) 
        THEN 
          Err 
            ( t 
            , "bitOp: unable to convert immediate to INTEGER:" 
              & TIntN . ToDiagnosticText ( index . imm ) 
            ) 
        END 
      ; ins . opcode := op . bitIndexInImm8Opcode1 
      ; build_modrm 
          ( t , bits , t . opcode [ op . bitIndexInImm8Opcode2 ] , ins ) 
      ; Err ( t , "bitOp: untested, non-working code reached #1" ) 
      ELSE 
        ins . opcode := op . bitIndexInRegOpcode 
      ; IF bits . loc = OLoc . mem 
        THEN 
...
; PROCEDURE incDecOp ( t : T ; READONLY op : Operand ; isDec : [ 0 .. 1 ] ) 
  = VAR ins : Instruction 
  ; BEGIN 
      Mn ( t , ARRAY [ 0 .. 1 ] OF TEXT { "INC" , "DEC" } [ isDec ] ) 
    ; MnOp ( t , op ) 
    ; 
...
; PROCEDURE unOp ( t : T ; op : Op ; READONLY dest : Operand ) 
  ...
  ; BEGIN 
      ...
      THEN 
        ...
        OF 
        | Op . oNOT 
          =>  unOp_double ( t , op , destA ) 
        | Op . oNEG 
          =>  negate_double ( t , destA ) 
        | Op . oSHL , Op . oSHR , Op . oSAR 
          =>  shift_double_ecx ( t , op , destA ) 
        ELSE 
          <* ASSERT FALSE *> 
        END 
      ELSE 
    ...
    ; IF src . loc = OLoc . imm 
      ...
      ; IF NOT TIntN . ToHostInteger ( src . imm , ins . imm ) 
        THEN 
          Err 
            ( t 
            , "imulOp: unable to convert immediate to INTEGER:" 
              & TIntN . ToDiagnosticText ( src . imm ) 
            ) 
        END 
      ; ins . imsize := 4 
...
; PROCEDURE imulImm 
    ( t : T ; READONLY dest , src : Operand ; imm : INTEGER ; imsize : CARDINAL ) 
  = VAR ins : Instruction 
  ; BEGIN 
                <* ASSERT dest.loc = OLoc.register *> 
<* ASSERT src.loc # OLoc.mem OR CG_Bytes[src.mvar.mvar_type] = 4 *> 
...
; PROCEDURE diffdivOp ( t : T ; READONLY divisor : Operand ; apos : BOOLEAN ) 
  = VAR diffsignlab := reserve_labels ( t , 1 , TRUE ) 
  ...
  ; BEGIN 
    ...
    ;                                                 (*   CDQ               *) 
      idivOp ( t , divisor ) 
    ;                                                 (*   IDIV EAX, divisor *) 
      immOp ( t , Op . oCMP , t . reg [ EDX ] , TZero ) 
    ;                                                 (*   CMP EDX, #0       *) 
...
(* ASSERT reg.loc = OLoc.register *) 
    ...
    ; IF ( NOT fully_known ) OR ( offset # 0 ) 
      THEN 
        ins . disp := offset 
      ; IF ( NOT fully_known ) OR ( offset > 16_7F ) 
           OR ( offset < - 16_80 ) 
        THEN 
          ins . dsize := 4 
        ; IF fully_known 
...
; PROCEDURE debugcode ( t : T ; READONLY ins : Instruction ) 
  ...
  ; BEGIN 
    ...
    ; IF ins . dsize # 0 
      THEN 
        HexLE ( t , ins . disp , ins . dsize ) 
      ; INC ( len , ins . dsize ) 
      END 
    ; IF ins . imsize # 0 
      THEN 
        HexLE ( t , ins . imm , ins . imsize ) 
      ; INC ( len , ins . imsize ) 
      END 
...
(*--------------------------------------------------------- jump routines ---*) 
  ...
  PROCEDURE case_jump 
    ( t : T ; READONLY index : Operand ; READONLY labels : ARRAY OF Label ) 
  = VAR ins : Instruction 
  ; BEGIN 
                <* ASSERT index.loc = OLoc.register *> 
      WITH curs = t . obj . cursor ( Seg . Text ) 
      DO 
        ins . opcode := 16_FF 
      ; ins . modrm := 16_24 
      ; ins . mrm_present := TRUE 
      ; ins . sib := 16_85 + index . reg [ 0 ] * 8 
      ; ins . sib_present := TRUE 
      ; ins . disp := curs + 7 
      ; ins . dsize := 4 
      ; writecode ( t , ins ) 
      ...
      ; FOR i := 0 TO NUMBER ( labels ) - 1 
        DO 
          check_label ( t , labels [ i ] , "case_jump" ) 
        ; WITH lab = t . labarr [ labels [ i ] ] 
          DO 
            IF lab . no_address 
            THEN 
            ...
            ; t . obj . relocate 
                ( t . textsym , curs + 7 + i * 4 , t . textsym ) 
            END 
          END 
        END 
      END 
  ...
  ; BEGIN 
    ...
    ; IF CG_Bytes [ type ] < 4 
      THEN 
        ...
        END 
      END 
    ; Mn ( t , mnemonic , " " , RegName [ r ] ) 
    ; MnPtr ( t , ind , offset , type ) 
    ; ins . mrm_present := TRUE 
    ; ins . modrm := r * 8 + ind . reg [ 0 ] 
    ; IF offset # 0 
      THEN 
        ins . disp := offset 
      ; IF offset > - 16_81 AND offset < 16_80 
        THEN 
          ins . dsize := 1 
        ; INC ( ins . modrm , 16_40 ) 
        ELSE 
...
; PROCEDURE store_ind1 
    ( t : T ; READONLY val , ind : Operand ; offset : ByteOffset ; type : MType ) 
  = VAR ins : Instruction 
  ; BEGIN 
    ...
    ; IF val . loc = OLoc . imm 
      THEN 
        ins . opcode := 16_C6 
      ; IF NOT TIntN . ToHostInteger ( val . imm , ins . imm ) 
        THEN 
          Err 
            ( t 
            , "store_ind1: unable to convert immediate to INTEGER:" 
              & TIntN . ToDiagnosticText ( val . imm ) 
            ) 
        END 
    ...
    ; IF offset # 0 
      THEN 
        ins . disp := offset 
      ; IF offset >= - 16_80 AND offset <= 16_7F 
        THEN 
          ins . dsize := 1 
        ; INC ( ins . modrm , 16_40 ) 
        ELSE 
...
<* ASSERT GetOperandSize(ind) = 1 *> 
    ...
    ; 
      ...
      ELSE 
        ...
        DO 
          store_ind1 
            ( t , valA [ i ] , ind , offset + i * 4 , valA [ i ] . optype ) 
        END 
      END 
    END store_ind 
 
; PROCEDURE f_loadind 
    ( t : T ; READONLY ind : Operand ; offset : ByteOffset ; type : MType ) 
  = VAR ins : Instruction 
  ; BEGIN 
                <* ASSERT ind.loc = OLoc.register *> 
...
; TYPE LabList 
    = OBJECT 
        seg : Seg ; offs : INTEGER ; abs : BOOLEAN ; link : LabList ; 
      END 
 
; PROCEDURE reserve_labels ( t : T ; n : INTEGER ; short := FALSE ) : Label 
  = VAR lab := t . next_label_id 
  ; BEGIN 
      IF t . next_label_id + n >= t . lablimit 
      THEN 
        expand_labels ( t ) 
      END 
    ; FOR i := lab TO lab + n - 1 
      DO 
...
; PROCEDURE expand_labels ( t : T ) 
  = VAR newarr := NEW ( REF ARRAY OF LLLabelTyp , t . lablimit * 2 ) 
  ; BEGIN 
      FOR i := 0 TO t . lablimit - 1 
      DO 
        newarr [ i ] := t . labarr [ i ] 
      END 
    ; t . labarr := newarr 
...
; PROCEDURE log_label_init ( t : T ; var : LLVarTyp ; offset : ByteOffset ; lab : Label ) 
  = BEGIN 
      check_label ( t , lab , "log_label_init" ) 
 
    ; t . obj . relocate ( var . symbol , offset , t . textsym ) 
 
    ; IF t . labarr [ lab ] . no_address 
      THEN 
        t . labarr [ lab ] . usage 
          := NEW 
               ( LabList 
               , seg := var . seg 
               , offs := t . obj . cursor ( var . seg ) 
               , abs := TRUE 
...
; PROCEDURE get_frame ( t : T ; r : Regno ; target , current : LLProcTyp ) 
  = BEGIN 
      IF current = target 
      THEN 
...
; PROCEDURE check_label ( t : T ; label : Label ; place : TEXT ) 
  = BEGIN 
      IF label >= t . next_label_id 
      THEN 
        Err ( t , "Tried to reference unknown label in " & place ) 
      END 
    END check_label 
 
; PROCEDURE fill_in_label_thread 
    ( t : T ; ptr : LabList ; val : INTEGER ; short : BOOLEAN ) 
  = BEGIN 
      WHILE ptr # NIL 
      DO 
        IF ptr . abs 
        ...
        ELSE 
          ...
          THEN 
            ...
            t . obj . patch 
              ( ptr . seg , ptr . offs , val - ( ptr . offs + 1 ) , 1 ) 
          ELSE 
            t . obj . patch 
              ( ptr . seg , ptr . offs , val - ( ptr . offs + 4 ) , 4 ) 
          END 
        END 
      ; ptr := ptr . link 
      END 
...
(*-------------------------------------------------- floating stack stuff ---*) 
  ...
  ; BEGIN 
      ...
      fstack_ensure ( t , 0 ) 
    ; 
      (* ensure will allow an extra space for the item 
    ...
    ; build_modrm 
        ...
        , t . opcode [ fopcode [ FOp . fLD ] . memop ] 
        , ins 
        ) 
    ; writecode ( t , ins ) 
    ; log_global_var ( t , t . ftop_mem , - 4 ) 
...
; PROCEDURE fstack_pop ( t : T ; READONLY mvar : MVar ) 
  ...
  ; BEGIN 
    ...
    ; build_modrm 
        ...
        , t . opcode [ fopcode [ FOp . fSTP ] . memop ] 
        , ins 
        ) 
    ; writecode ( t , ins ) 
    ; log_global_var ( t , mvar , - 4 ) 
...
; PROCEDURE fstack_swap ( t : T ) 
  ...
  ; BEGIN 
    ...
    ; Mn ( t , "FLD ST, m80real" ) 
    ; build_modrm 
        ( t , t . fstackspill [ t . fspilltop - 2 ] , t . opcode [ 5 ] , ins ) 
    ; ins . opcode := 16_DB 
    ; writecode ( t , ins ) 
    ...
    ; ins := Instruction { } 
    ; build_modrm 
        ( t , t . fstackspill [ t . fspilltop - 1 ] , t . opcode [ 5 ] , ins ) 
    ; ins . opcode := 16_DB 
    ; writecode ( t , ins ) 
...
; PROCEDURE fstack_wipeup ( t : T ; wipeup : INTEGER ) 
  = BEGIN 
      IF wipeup + t . fstackloaded > 8 
      THEN 
        Err ( t , "Stack overflow in fstack_wipeup" ) 
      END 
    ; IF wipeup > t . fspilltop 
      THEN 
        Err 
          ( t , "Not enough spilled fstack elements to replace in fstack_wipeup" ) 
      END 
    ; FOR i := 1 TO wipeup 
      DO 
        retrieve_temp ( t ) 
...
(*------------------------------------------------------- code writing i/o---*) 
  ...
  = BEGIN 
      ...
      THEN 
        MnOp ( t , op ) 
      ; Mn ( t , "^[" , Fmt . Int ( disp ) ) 
      ; IF ( type # Type . Int32 ) AND ( type # Type . Word32 ) 
        THEN 
  ...
  = BEGIN 
      ...
      THEN 
        ...
        OF OLoc . fstack 
          =>  Mn ( t , " FST" ) 
        | OLoc . register 
          =>  Mn ( t , " " , RegName [ op . reg [ 0 ] ] ) 
        | OLoc . imm 
          =>  Mn ( t , " $" , TIntN . ToText ( op . imm ) ) 
        | OLoc . mem 
          =>  MnMVar ( t , op . mvar ) 
        END 
      END 
...
; PROCEDURE MnMVar ( t : T ; READONLY mvar : MVar ) 
  = BEGIN 
      ...
      THEN 
        MnVar ( t , mvar . var ) 
      ; IF mvar . mvar_offset # 0 
        THEN 
          Mn ( t , "+" , Fmt . Int ( mvar . mvar_offset ) ) 
        END 
      ; IF ( mvar . mvar_type # Type . Int32 ) 
           AND ( mvar . mvar_type # Type . Word32 ) 
        THEN 
...
; PROCEDURE MnVar ( t : T ; READONLY v : LLVarTyp ) 
...
; PROCEDURE MnProc ( t : T ; LLProc : LLProcTyp ) 
  = BEGIN 
      IF t . debug 
      THEN 
        IF ( LLProc = NIL ) 
        THEN 
          Mn ( t , " *" ) 
        ELSE 
          Mn ( t , " p." , Fmt . Int ( LLProc . tag ) ) 
        ; IF LLProc . name # M3ID . NoID 
          THEN 
            Mn ( t , "[" , M3ID . ToText ( LLProc . name ) , "]" ) 
          END 
        END 
      END 
    END MnProc 
...
; PROCEDURE Mn ( t : T ; mn1 , mn2 , mn3 : TEXT := NIL ) 
  = BEGIN 
      IF t . debug 
      THEN 
        IF ( mn1 # NIL ) AND ( t . n_tags < NUMBER ( t . tags ) ) 
        THEN 
          t . tags [ t . n_tags ] := mn1 
        ; INC ( t . n_tags ) 
        END 
      ; IF ( mn2 # NIL ) AND ( t . n_tags < NUMBER ( t . tags ) ) 
        THEN 
          t . tags [ t . n_tags ] := mn2 
        ; INC ( t . n_tags ) 
        END 
      ; IF ( mn3 # NIL ) AND ( t . n_tags < NUMBER ( t . tags ) ) 
        THEN 
          t . tags [ t . n_tags ] := mn3 
        ; INC ( t . n_tags ) 
        END 
...
(*--------------------------------------------------- temporary var stuff ---*) 
  ...
  = VAR ins : Instruction 
  ; BEGIN 
      IF t . fspilltop = t . fspilllimit 
      THEN 
        expand_spill ( t ) 
      END 
    ; WITH spill = t . fstackspill [ t . fspilltop ] 
      DO 
        IF t . fspilltop = t . fspillhigh 
        THEN 
          spill . loc := OLoc . mem 
        ; spill . mvar . var 
            := t . parent . declare_temp ( 10 , 4 , Type . Void , FALSE ) 
        ; INC ( t . fspillhigh ) 
        END 
...
(*---------------------------------------------- future update list stuff ---*) 
  ...
  ; BEGIN 
      IF mvar . var . loc # VLoc . global 
      THEN 
        RETURN 
      END 
 
    ; patch_loc := t . obj . cursor ( Seg . Text ) + reltocurs 
    ; IF mvar . var = t . flitvar 
      THEN 
      ...
      ELSE 
        t . obj . patch 
          ( Seg . Text , patch_loc , mvar . mvar_offset + mvar . var . offset , 4 ) 


(* This crashed relocate with t.textsym=0: 
...
(*---------------------------------------------------------------------------*) 
  ...
  = BEGIN 
    ...
    ; t . flitvar := t . parent . NewLLVar ( Type . Struct , 0 , 0 , 4 ) 
...
; TYPE LocList = REF RECORD loc : ByteOffset ; link : LocList ; END 
 
; PROCEDURE find_flit 
    (<*UNUSED*> t : T 
    ; READONLY flarr : FloatBytes 
    ; size : INTEGER 
    ; used : FLiteral 
    ; VAR loc : ByteOffset 
    ) 
  ...
  ; BEGIN 
      ...
      DO 
        i := 0 
      ; WHILE ( i < size ) AND ( flarr [ i ] = used . arr [ i ] ) 
        DO 
          INC ( i ) 
        END 
      ; IF ( i = size ) 
...
*) 
      ...
      ; 
        ...
        WHILE abscallptr # NIL 
        DO 
          t . obj . patch 
            ( Seg . Text , abscallptr . loc , abscall_locs . loc , 4 ) 
(* This crashed relocate with t.textsym=0
        ; t . obj . relocate 
            ( t . textsym , abscallptr . loc , intvar . symbol ) 
*) 
...
; PROCEDURE log_flit_use 
    ( t : T ; VAR internal_size : INTEGER ; VAR flloc : LocList ) 
  : FLiteral 
  ...
  ; BEGIN 
      ...
      DO 
        IF NOT find_flit 
             ( t , flptr . arr , flptr . flit_size , f_lit , f_litloc ) 
        THEN 
...
; PROCEDURE log_abscall_use 
    ( t : T ; VAR internal_size : INTEGER ; VAR abscallloc : LocList ) 
  : AbsCall 
  ...
  ; BEGIN 
      WHILE abscallptr # NIL 
      DO 
        IF NOT find_abscall ( t , abscallptr . sym , abscall , abcloc ) 
        THEN 
...
; PROCEDURE init_intvar 
    ( t : T ; size : ByteSize ; f_lit : FLiteral ; abscall : AbsCall ) 
  : LLVarTyp 
  = VAR intvar : LLVarTyp 
  ;   tint : Target . Int 
  ; BEGIN 
      intvar 
        := t . parent . declare_global 
             ( M3ID . NoID , size , 4 , Type . Struct , 0 , FALSE , TRUE ) 
    ; t . parent . begin_init ( intvar ) 
 
    ; WHILE f_lit # NIL 
      DO 
        FOR i := 0 TO f_lit . flit_size - 1 
        DO 
...
; PROCEDURE New ( parent : LLTypes . CodeGenTyp ; wr : LLWr . T ) : T 
  ...
  ; BEGIN 
    ...
    ; code . lablimit := 256 
    ; code . labarr := NEW ( REF ARRAY OF LLLabelTyp , code . lablimit ) 
