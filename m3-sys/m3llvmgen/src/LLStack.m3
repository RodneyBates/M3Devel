...
; IMPORT LLCode 
; FROM LLCode IMPORT Op , FOp , Cond , revcond 
; IMPORT LLTypes 
; FROM LLTypes 
    IMPORT Operand , MVar , Regno , OLoc , VLoc , NRegs , Force , TypeIs64 
    , OperandPart , RegName , OperandSize , TZero , AllRegisters 
; FROM LLTypes 
    IMPORT RegistersForByteOperations , RegSet , FlToInt , LLVarTyp , LLProcTyp 
    , NoStore , SplitOperand , SplitMVar , GetTypeSize , GetOperandSize 
; FROM LLTypes 
    IMPORT TypeIsSigned , TypeIsUnsigned , EAX , ECX , EDX , EBX , UnsignedType 
    , MaximumShift , MinimumShift , BitCountMask , IntType 
; IMPORT LLWr 
; IMPORT M3CG 
...
; REVEAL T 
    = Public BRANDED "LLStack.T" OBJECT 
        cg : LLCode . T := NIL 
 
 
      ; parent : LLTypes . CodeGenTyp := NIL 
...
; PROCEDURE check ( t : T ; where : TEXT ) 
  ...
  ; BEGIN 
    ...
    ; FOR i := 0 TO NRegs 
      DO 
        IF t . reguse [ i ] . stackp # - 1 
        THEN 
          IF t . vstack [ t . reguse [ i ] . stackp ] . reg 
               [ t . reguse [ i ] . operandPart ] 
             # i 
          THEN 
            Err 
              ( t 
              , where & " i:" & RegName [ i ] & " t.reguse[i].stackp:" 
                & Fmt . Int ( t . reguse [ i ] . stackp ) 
                & " t.vstack[t.reguse[i].stackp].reg[t.reguse[i].operandPart]:" 
                & RegName 
                  [ t . vstack [ t . reguse [ i ] . stackp ] . reg 
                      [ t . reguse [ i ] . operandPart ] 
                  ] 
              ) 
          END 
    ...
    ; FOR i := 0 TO t . stacktop - 1 
      DO 
        IF t . vstack [ i ] . loc = OLoc . register 
        THEN 
          size := GetOperandSize ( t . vstack [ i ] ) 
        ; FOR j := 0 TO size - 1 
          DO 
            IF t . reguse [ t . vstack [ i ] . reg [ j ] ] . stackp # i 
            THEN 
              Err 
                ( t 
                , where & " i:" & Fmt . Int ( i ) & " j:" & Fmt . Int ( j ) 
                  & " t.vstack[i].reg[j]:" 
                  & RegName [ t . vstack [ i ] . reg [ j ] ] 
                  & " t.reguse[t.vstack[i].reg[j]].stackp:" 
                  & Fmt . Int 
                    ( t . reguse [ t . vstack [ i ] . reg [ j ] ] . stackp ) 
                ) 
            END 
...
; PROCEDURE loadreg 
    ( t : T ; r : Regno ; READONLY op : Operand ; operandPart : OperandPart ) 
  = BEGIN 
...
; PROCEDURE loadphantom 
    ( t : T ; r : Regno ; stackp : CARDINAL ; operandPart : OperandPart ) 
  = BEGIN 
...
; PROCEDURE copyreg 
    ( t : T ; stackp : CARDINAL ; to , from : Regno ; operandPart : OperandPart ) 
  = BEGIN 
...
; PROCEDURE swapreg ( t : T ; to , from : Regno ; operandPart : OperandPart ) 
  ...
  ; BEGIN 
    ...
    ; IF t . reguse [ from ] . stackp # - 1 
      THEN 
        set_reg 
          ( t 
          , t . reguse [ from ] . stackp 
          , from 
          , t . reguse [ from ] . operandPart 
          ) 
      END 
...
; PROCEDURE clearall ( t : T ) 
  = BEGIN 
    ...
    ; FOR r := 0 TO NRegs 
      DO 
        IF t . reguse [ r ] . stackp # - 1 
        THEN 
          Err 
            ( t 
            , "r:" & RegName [ r ] & " t.reguse[r].stackp:" 
              & Fmt . Int ( t . reguse [ r ] . stackp ) 
            ) 
        END 
 ...
 *) 
    VAR in : ARRAY OperandPart OF Regno 
  ; (* initialize to -1? *) 
      to : ARRAY OperandPart OF Regno 
  ; (* initialize to -1? *) 
  ...
  ; BEGIN 
      ...
      ; 
        ...
        THEN 
          ret64 
            := ( force = Force . regset AND set = RegSet { EAX , EDX } ) 
        ; compare_exchange_64 
            := ( force = Force . regset AND set = RegSet { ECX , EBX } ) 
        END 
          ...
          | OLoc . imm 
            =>  IF i = 0 
                ...
                ELSE 
                  in [ i ] 
                    := immreg 
                         ( t , opA [ i ] . imm , set - RegSet { in [ 0 ] } ) 
                END 
          END 
        END 
 
      ; IF op . mvar . mvar_type = Type . Addr 
        THEN 
          hintaddr := TRUE 
        END 
      ...
      ; 
        ...
        IF force = Force . mem AND op . loc = OLoc . register 
        THEN 
          get_temp ( t , stackp ) 
        ; RETURN 
        END 
      ...
      ; 
        ...
        IF force = Force . mem AND op . loc = OLoc . imm 
        THEN 
          get_temp ( t , stackp ) 
        ; RETURN 
        END 
      ...
      ; 
        ...
        IF op . loc = OLoc . imm AND force # Force . anyreg 
           AND force # Force . regset 
        THEN 
          RETURN 
        END 
      ...
      ; 
        ...
        IF force = Force . any AND in [ 0 ] = - 1 AND in [ size - 1 ] = - 1 
        THEN 
          RETURN 
        END 
 
      ; IF force = Force . anydword AND in [ 0 ] = - 1 AND in [ size - 1 ] = - 1 
           AND op . loc = OLoc . mem 
           AND CG_Bytes [ op . mvar . mvar_type ] = ( size * 4 ) 
        THEN 
          RETURN 
        END 
      ...
      ; 
        ...
        IF force = Force . anytemp AND in [ 0 ] = - 1 AND in [ size - 1 ] = - 1 
           AND op . loc = OLoc . mem 
           AND op . mvar . var . stack_temp 
        THEN 
          RETURN 
        END 
 
      ; IF op . loc = OLoc . mem AND CG_Bytes [ op . mvar . mvar_type ] = 1 
        THEN 
          force := Force . regset 
      ...
      ; 
        ...
        DO 
          IF ( in [ i ] = - 1 ) 
             OR ( force = Force . regset AND ( NOT in [ i ] IN set ) ) 
             ...
             *) 
             ...
             OR ( t . reguse [ in [ i ] ] . stackp # - 1 
               AND t . reguse [ in [ i ] ] . stackp # stackp 
               ) 
             OR ( t . reguse [ in [ i ] ] . stackp = stackp 
               AND t . reguse [ in [ i ] ] . operandPart # i 
               ) 
          THEN 
            ...
            THEN 
              to [ 0 ] := EAX 
            ; to [ 1 ] := EDX 
            ELSIF compare_exchange_64 
            THEN 
              to [ 0 ] := EBX 
            ; to [ 1 ] := ECX 
            ; t . parent . proc_reguse [ EBX ] := TRUE 
            ELSIF i = 1 AND done [ 0 ] = FALSE 
            THEN 
       ...
       *) 
        ...
        IF size = 2 AND in [ 0 ] = - 1 AND in [ 1 ] = - 1 AND to [ 1 ] = EAX 
           AND NOT done [ 0 ] 
           AND NOT done [ 1 ] 
        THEN 
          to [ 1 ] := to [ 0 ] 
      ...
      ; 
        ...
        IF force = Force . any AND t . reguse [ to [ 0 ] ] . stackp # - 1 
           AND t . reguse [ to [ size - 1 ] ] . stackp # - 1 
        THEN 
          RETURN 
        END 
 
      ; IF force = Force . anydword AND op . loc = OLoc . mem 
      ...
      ; 
        ...
        IF force = Force . anytemp AND t . reguse [ to [ 0 ] ] . stackp # - 1 
           AND t . reguse [ in [ size - 1 ] ] . stackp # - 1 
           AND op . loc = OLoc . mem 
           AND op . mvar . var . stack_temp 
        THEN 
          RETURN 
       ...
       *) 
        ...
        THEN 
          VAR do_loads := FALSE 
        ; BEGIN 
            IF NOT done [ 0 ] AND NOT done [ 1 ] AND in [ 0 ] = to [ 1 ] 
               AND in [ 1 ] = to [ 0 ] 
            THEN 
              do_loads := TRUE 
            ; done [ 0 ] := TRUE 
          ...
          (* Some of these might be better as "copy" or "move" but should be ok. *) 
            ELSIF NOT done [ 1 ] AND in [ 0 ] = to [ 1 ] 
            THEN 
              do_loads := TRUE 
            ; done [ 1 ] := TRUE 
            ELSIF NOT done [ 0 ] AND in [ 1 ] = to [ 0 ] 
            THEN 
              do_loads := TRUE 
            ; done [ 0 ] := TRUE 
            END 
      ...
      ; 
        ...
        DO 
          ...
          THEN 
            ...
            IF t . reguse [ to [ i ] ] . stackp = - 1 
            THEN 
              IF in [ i ] = - 1 
              THEN 
                loadreg ( t , to [ i ] , opA [ i ] , i ) 
              ELSE 
                IF t . reguse [ in [ i ] ] . stackp = stackp 
                THEN 
...
; PROCEDURE pickreg ( t : T ; set : RegSet := AllRegisters ; hintaddr := FALSE ) : Regno 
  ...
  ; BEGIN 
      ...
      DO 
        IF i IN set 
        THEN 
          WITH prec = precedence ( t , i , hintaddr ) 
          DO 
            IF prec < minprec 
            THEN 
              minprec := prec 
...
; PROCEDURE inreg ( t : T ; READONLY v : MVar ; set : RegSet := AllRegisters ) : Regno 
  ...
  ; BEGIN 
    ...
    ; FOR i := 0 TO NRegs 
      DO 
        IF t . reguse [ i ] . last_store # NoStore 
           AND v = t . reguse [ i ] . last_store 
        THEN 
          prec := precedence ( t , i ) 
        ; IF NOT i IN set 
          THEN 
...
; PROCEDURE immreg 
    ( t : T ; READONLY imm : TIntN . T ; set : RegSet := AllRegisters ) 
  : Regno 
  = VAR minprec := HighPrec * HighPrec 
  ;   prec := 0 
  ;   bestreg : Regno := - 1 
  ; BEGIN 
      ...
      DO 
        IF t . reguse [ i ] . imm 
           AND TIntN . EQ ( imm , t . reguse [ i ] . last_imm ) 
        THEN 
          prec := precedence ( t , i ) 
        ; IF NOT i IN set 
          THEN 
...
; CONST baseprec 
    = ARRAY BOOLEAN , [ 0 .. NRegs ] OF INTEGER 
        { ARRAY [ 0 .. NRegs ] OF INTEGER 
            { 6 , 5 , 2 , 1 , HighPrec , HighPrec , 3 , 4 } 
        , ARRAY [ 0 .. NRegs ] OF INTEGER 
            { 6 , 5 , 4 , 3 , HighPrec , HighPrec , 1 , 2 } 
        } 
 
; PROCEDURE precedence ( t : T ; r : Regno ; hintaddr := FALSE ) : INTEGER 
  = VAR prec : INTEGER 
  ; BEGIN 
      IF baseprec [ hintaddr ] [ r ] = HighPrec 
      THEN 
        RETURN HighPrec 
      END 
    ; IF t . reguse [ r ] . locked 
      THEN 
        RETURN HighPrec 
      END 
    ; IF t . reguse [ r ] . stackp # - 1 
      THEN 
        prec := 4 * NRegs 
      ELSIF t . reguse [ r ] . last_store # NoStore 
      THEN 
        prec := 3 * NRegs 
      ELSIF t . reguse [ r ] . imm 
      THEN 
        prec := 2 * NRegs 
      ELSE 
        prec := NRegs 
...
(*-------------------------------------------------------- stack routines ---*) 
  ...
  = VAR op := t . vstack [ stackp ] 
  ; (* *copy* this before changing it *) 
  ...
  ; BEGIN 
      set_mvar 
        ...
        , MVar 
            { var 
                := t . parent . declare_temp 
                     ( size * 4 , 4 , op . optype , FALSE ) 
            , mvar_offset := 0 
            , mvar_type := op . optype 
            } 
        ) 
    ...
    ; CASE op . loc 
      ...
      | OLoc . register 
        =>  mvar := t . vstack [ stackp ] . mvar 
          ;                                             (* temporarily save away *) 
            t . vstack [ stackp ] . mvar . mvar_type := Type . Word32 
          ; (* temporarily make it register sized *) 
          ...
          ; FOR i := 0 TO size - 1 
            ...
            ;                                           (* temporary offset to register sized chunk *) 
              t . cg . movOp 
                ( t . vstack [ stackp ] , t . cg . reg [ op . reg [ i ] ] ) 
            END 
 ...
 *) 
  ...
  ; BEGIN 
      FOR i := 0 TO t . stacktop - 1 
      DO 
        IF t . vstack [ i ] . loc = OLoc . mem 
           AND t . vstack [ i ] . mvar = mvar 
        THEN 
          IF NOT doneswap 
          THEN 
            doneswap := TRUE 
        ...
        ; set_mvar 
            ...
            , MVar 
                { var 
                    := t . parent . declare_temp ( size * 4 , 4 , type , FALSE ) 
                , mvar_offset := 0 
                , mvar_type := type 
                } 
            ) 
...
; PROCEDURE dealloc_reg ( t : T ; stackp : CARDINAL ; operandPart : OperandPart ) 
  = BEGIN 
    ...
    ; t . reguse [ t . vstack [ stackp ] . reg [ operandPart ] ] . operandPart 
        := - 1 
    END dealloc_reg 
 
; PROCEDURE corrupt ( t : T ; reg : Regno ; operandPart : OperandPart ) 
  = BEGIN 
      IF t . reguse [ reg ] . stackp # - 1 
      THEN 
        forceout ( t , reg , operandPart ) 
      END 
    ; t . parent . proc_reguse [ reg ] := TRUE 
    ; t . reguse [ reg ] 
        := InitRegister ( locked := t . reguse [ reg ] . locked ) 
    END corrupt 
...
; PROCEDURE pushimmT ( t : T ; imm : TIntN . T ; type : Type ) 
  = BEGIN 
    ...
    ; WITH stack0 = t . vstack [ t . stacktop ] 
      DO 
        stack0 . loc := OLoc . imm 
      ; stack0 . imm := imm 
...
; PROCEDURE pushimmI ( t : T ; immI : INTEGER ; type : Type ) 
  = VAR immT : TIntN . T 
  ; BEGIN 
      IF NOT TIntN . FromHostInteger ( immI , Target . Integer . bytes , immT ) 
      THEN 
  ...
  ; BEGIN 
    ...
    ; FOR i := 0 TO size - 1 
      DO 
        ...
        IF size = 2 AND i = 0 AND force = Force . regset 
           AND set = RegSet { EAX , EDX } 
        THEN 
          r := pickreg ( t , RegSet { EAX } , hintaddr ) 
        ELSIF size = 2 AND i = 0 AND force = Force . regset 
           AND set = RegSet { ECX , EBX } 
        THEN 
    ...
    ; WITH stack0 = t . vstack [ t . stacktop ] 
      ...
      ; IF FloatType [ type ] 
        ...
        ELSE 
          IF force = Force . mem OR ( any_reg_in_use AND force = Force . any ) 
          THEN 
            set_mvar 
              ( t 
              , t . stacktop 
...
; PROCEDURE push ( t : T ; READONLY src_mvar : MVar ) 
  ...
  ; BEGIN 
    ...
    ; WITH stack0 = t . vstack [ t . stacktop ] 
      ...
      ; IF FloatType [ src_mvar . mvar_type ] 
        THEN 
          IF src_mvar . var . loc = VLoc . temp 
             AND src_mvar . var . parent # t . current_proc 
          THEN 
            unlock ( t ) 
          ...
          ; t . cg . get_frame 
              ( indreg , src_mvar . var . parent , t . current_proc ) 
          ; t . cg . f_loadind 
              ( t . cg . reg [ indreg ] 
              , src_mvar . mvar_offset + src_mvar . var . offset 
              , src_mvar . mvar_type 
              ) 
          ; stack0 . loc := OLoc . fstack 
          ELSE 
        ...
        ELSE 
          ...
          ; FOR i := 0 TO srcSize - 1 
            DO 
              IF CG_Bytes [ src_mvar . mvar_type ] = 1 
              THEN 
              ...
              ELSE 
                destreg [ i ] 
                  := pickreg 
                       ( t , AllRegisters , src_mvar . mvar_type = Type . Addr ) 
              END 
          ...
          ; FOR i := 0 TO srcSize - 1 
            DO 
              t . cg . load_ind 
                ( destreg [ i ] 
                , t . cg . reg [ indreg ] 
                , src_mvar . mvar_offset + src_mvar . var . offset + i * 4 
                , src_mvar . mvar_type 
                ) 
...
; PROCEDURE pop ( t : T ; READONLY dest_mvar : MVar ) 
  ...
  ; BEGIN 
    ...
    ; WITH src_stack0 = t . vstack [ t . stacktop - 1 ] 
      DO 
        IF src_stack0 . loc = OLoc . fstack 
        THEN 
          IF dest_mvar . var . loc = VLoc . temp 
             AND dest_mvar . var . parent # t . current_proc 
          THEN 
            unlock ( t ) 
          ; indreg := pickreg ( t , AllRegisters , TRUE ) 
          ; corrupt ( t , indreg , operandPart := 0 ) 
          ; t . cg . get_frame 
              ( indreg , dest_mvar . var . parent , t . current_proc ) 
          ; t . cg . f_storeind 
              ( t . cg . reg [ indreg ] 
              , dest_mvar . mvar_offset + dest_mvar . var . offset 
              , dest_mvar . mvar_type 
              ) 
        ...
        ELSE 
          unlock ( t ) 
        ; IF CG_Bytes [ dest_mvar . mvar_type ] = 1 
             AND src_stack0 . loc # OLoc . imm 
          THEN 
            find 
              ( t 
              , t . stacktop - 1 
        ...
        ; IF srcSize # destSize 
          THEN 
            Err 
              ( t 
              , " srcSize:" & Fmt . Int ( srcSize ) & " destSize:" 
                & Fmt . Int ( destSize ) 
                & " dest_mvar.mvar_type:" 
                & Target . TypeNames [ dest_mvar . mvar_type ] 
                & " src_stack0.optype:" 
                & Target . TypeNames [ src_stack0 . optype ] 
              ) 
          END 
          ...
          ; FOR i := 0 TO destSize - 1 
            DO 
              t . cg . store_ind 
                ( src_opA [ i ] 
                , t . cg . reg [ indreg ] 
                , dest_mvarA [ i ] . mvar_offset 
                  + dest_mvarA [ i ] . var . offset 
          ...
          ; FOR i := 0 TO NRegs 
            DO 
              IF t . reguse [ i ] . last_store = dest_mvar 
                 ...
                 OR t . reguse [ i ] . last_store = dest_mvarA [ destSize - 1 ] 
              THEN 
                t . reguse [ i ] . last_store := NoStore 
              END 
            END 
 
          ; IF src_stack0 . loc = OLoc . register 
            THEN 
              FOR i := 0 TO destSize - 1 
              DO 
                t . reguse [ src_stack0 . reg [ i ] ] . stackp := - 1 
              ; t . reguse [ src_stack0 . reg [ i ] ] . operandPart := - 1 
              ; t . reguse [ src_stack0 . reg [ i ] ] . last_store 
                  := dest_mvarA [ i ] 
              END 
            END 
...
; PROCEDURE doloadaddress ( t : T ; v : LLVarTyp ; o : ByteOffset ) 
  = VAR to , tvoffset , ti : TIntN . T 
  ; BEGIN 
      unlock ( t ) 
    ; pushnew ( t , Type . Addr , Force . anyreg ) 
 
    ; WITH stop0 = t . vstack [ pos ( t , 0 , "doloadaddress" ) ] 
      DO 
        IF v . loc = VLoc . temp AND v . parent # t . current_proc 
        THEN 
          t . cg . get_frame 
            ( stop0 . reg [ 0 ] , v . parent , t . current_proc ) 
        ; IF NOT TIntN . FromHostInteger ( o , Target . Integer . bytes , to ) 
          THEN 
            Err ( t , "doloadaddress: unable to convert o" ) 
          END 
        ; IF NOT TIntN . FromHostInteger 
               ( v . offset , Target . Integer . bytes , tvoffset ) 
          THEN 
            Err ( t , "doloadaddress: unable to convert v.offset" ) 
          END 
        ; IF NOT TIntN . Add ( to , tvoffset , ti ) 
          THEN 
...
; PROCEDURE findbin 
    ( t : T ; symmetric , overwritesdest : BOOLEAN ; VAR dest , src : INTEGER ) 
  : BOOLEAN 
  = VAR reversed := FALSE 
  ; BEGIN 
      ...
      ; WITH stop0 = t . vstack [ stack0 ] , stop1 = t . vstack [ stack1 ] 
        DO 
 
          IF GetTypeSize ( stop0 . optype ) # GetTypeSize ( stop1 . optype ) 
          THEN 
            Err 
              ( t 
              , "findbin: stop0.optype:" & Target . TypeNames [ stop0 . optype ] 
                & " stop1.optype:" 
                & Target . TypeNames [ stop1 . optype ] 
              ) 
          END 
 
        ; IF symmetric 
          THEN 
            IF stop0 . loc = OLoc . register OR stop1 . loc = OLoc . imm 
               OR ( stop0 . loc = OLoc . mem AND stop0 . mvar . var . stack_temp 
                 AND stop1 . loc # OLoc . register 
                 ) 
            THEN 
              dest := stack0 
            ; src := stack1 
    ...
    ; WITH destop = t . vstack [ dest ] , srcop = t . vstack [ src ] 
      DO 
      ...
      ; IF destop . loc = OLoc . imm 
        THEN 
          find ( t , dest , Force . anyreg ) 
        END 
 
      ; IF destop . loc = OLoc . mem 
           AND ( CG_Bytes [ destop . mvar . mvar_type ] < 4 
                 OR srcop . loc = OLoc . mem 
               ) 
        THEN 
          find ( t , dest , Force . anyreg ) 
        END 
 
      ; IF srcop . loc = OLoc . mem 
           AND CG_Bytes [ srcop . mvar . mvar_type ] < 4 
        THEN 
...
; PROCEDURE dobin 
    ( t : T ; op : Op ; symmetric , overwritesdest : BOOLEAN ; type : Type ) 
  : BOOLEAN 
    ...
    ; 
      ...
      DO 
        ...
        IF ( GetTypeSize ( destop . optype ) # GetTypeSize ( srcop . optype ) ) 
           OR ( GetTypeSize ( destop . optype ) # GetTypeSize ( type ) ) 
        THEN 
          Err 
            ( t 
            , " GetTypeSize(destop.optype):" 
              & Fmt . Int ( GetTypeSize ( destop . optype ) ) 
              & " GetTypeSize(srcop.optype):" 
              & Fmt . Int ( GetTypeSize ( srcop . optype ) ) 
              & " GetTypeSize(type):" 
              & Fmt . Int ( GetTypeSize ( type ) ) 
              & " destop.optype:" 
              & Target . TypeNames [ destop . optype ] 
              & " srcop.optype:" 
              & Target . TypeNames [ srcop . optype ] 
              & " type:" 
              & Target . TypeNames [ type ] 
            ) 
        END 
...
; PROCEDURE dostoreind ( t : T ; o : ByteOffset ; type : MType ) 
  = BEGIN 
      ...
      , stack1 = pos ( t , 1 , "store_indirect" ) 
      DO 
        find ( t , stack1 , Force . any , AllRegisters , TRUE ) 
      ; IF CG_Bytes [ type ] = 1 AND t . vstack [ stack0 ] . loc # OLoc . imm 
        THEN 
      ...
      ; IF t . vstack [ stack1 ] . loc # OLoc . register 
        THEN 
          find ( t , stack1 , Force . anyreg , AllRegisters , TRUE ) 
        END 
 
      ; t . cg . store_ind 
          ( t . vstack [ stack0 ] , t . vstack [ stack1 ] , o , type ) 
      END 
...
; PROCEDURE doumul ( t : T ) 
  ...
  ; BEGIN 
      unlock ( t ) 
    ; WITH stack0 = pos ( t , 0 , "doumul" ) , stack1 = pos ( t , 1 , "doumul" ) 
      DO 
        WITH stop0 = t . vstack [ stack0 ] , stop1 = t . vstack [ stack1 ] 
        DO 
          IF stop0 . loc = OLoc . register AND stop0 . reg [ 0 ] = EAX 
          THEN 
            lock ( t , EAX ) 
          ; find ( t , stack1 , Force . anydword ) 
          ; otherop := stop1 
          ELSIF stop1 . loc = OLoc . register AND stop1 . reg [ 0 ] = EAX 
          THEN 
            lock ( t , EAX ) 
          ; find ( t , stack0 , Force . anydword ) 
          ; otherop := stop0 
          ELSIF stop0 . loc = OLoc . register 
          THEN 
      ...
      ; IF otherop . loc = OLoc . imm 
        THEN 
          IF otherop = t . vstack [ stack1 ] 
          THEN 
      ...
      ; IF otherop . loc # OLoc . register OR otherop . reg [ 0 ] # EDX 
        THEN 
...
; PROCEDURE doimul ( t : T ) 
  ...
  ; BEGIN 
      unlock ( t ) 
    ; WITH stack0 = pos ( t , 0 , "doimul" ) , stack1 = pos ( t , 1 , "doimul" ) 
      DO 
        WITH stop0 = t . vstack [ stack0 ] , stop1 = t . vstack [ stack1 ] 
        DO 
          find ( t , stack1 , Force . any ) 
        ; IF stop1 . loc = OLoc . register 
          THEN 
            find ( t , stack0 , Force . anydword ) 
          ; dest := stop1 
          ; src := stop0 
...
; PROCEDURE dodiv ( t : T ; a , b : Sign ) 
  ...
  ; BEGIN 
    ...
    ; WITH stack0 = pos ( t , 0 , "dodiv" ) , stack1 = pos ( t , 1 , "dodiv" ) 
      DO 
        find ( t , stack1 , Force . regset , RegSet { EAX } ) 
 
      ; IF a # Sign . Unknown AND b # Sign . Unknown 
        THEN 
          find ( t , stack0 , Force . anydword ) 
        ; IF t . vstack [ stack0 ] . loc = OLoc . imm 
          THEN 
            find ( t , stack0 , Force . anyreg ) 
          END 
        ...
        ; IF a = Sign . Positive 
          THEN 
            t . cg . binOp 
              ( Op . oXOR , t . cg . reg [ EDX ] , t . cg . reg [ EDX ] ) 
          ELSE 
            t . cg . noargOp ( Op . oCDQ ) 
          END 
 
        ; IF a = Sign . Positive AND b = Sign . Positive 
          THEN 
        ...
        ; IF ( a = Sign . Positive AND b = Sign . Negative ) 
             OR ( a = Sign . Negative AND b = Sign . Positive ) 
          THEN 
    ...
    ; WITH stack0 = pos ( t , 0 , "domod" ) 
      ...
      DO 
       ...
       <* ASSERT GetTypeSize(stop0.optype) = GetTypeSize(stop1.optype) *> 
        ...
        find ( t , stack1 , Force . regset , RegSet { EAX } ) 
      ; IF ( a = Sign . Positive AND b = Sign . Positive ) 
           OR ( a = Sign . Negative AND b = Sign . Negative ) 
        THEN 
          find ( t , stack0 , Force . anydword ) 
        ; IF t . vstack [ stack0 ] . loc = OLoc . imm 
          THEN 
      ...
      ; IF a # Sign . Unknown AND b # Sign . Unknown 
        ...
        ; IF ( a = Sign . Positive AND b = Sign . Negative ) 
          ...
          ; t . cg . brOp ( Cond . E , neglabel ) 
          ; t . cg . binOp 
              ( Op . oADD , t . cg . reg [ EDX ] , t . vstack [ stack0 ] ) 
...
; PROCEDURE doimm 
    ( t : T ; op : Op ; READONLY imm : TIntN . T ; overwritesdest : BOOLEAN ) 
  = BEGIN 
      unlock ( t ) 
 
    ; WITH stack0 = pos ( t , 0 , "doimm" ) , stop0 = t . vstack [ stack0 ] 
      DO 
        IF ( stop0 . loc = OLoc . mem 
             AND ( ( overwritesdest AND NOT stop0 . mvar . var . stack_temp ) 
                   OR CG_Bytes [ stop0 . mvar . mvar_type ] = 2 
                   OR ( CG_Bytes [ stop0 . mvar . mvar_type ] = 1 
                     AND ( TIntN . GT ( imm , TIntN . Max8 ) 
                           OR TIntN . LT ( imm , TIntN . Min8 ) 
                         ) 
                     ) 
                 ) 
             ) 
           OR stop0 . loc = OLoc . imm 
        THEN 
          find ( t , stack0 , Force . anyreg ) 
        ELSE 
          find ( t , stack0 , Force . any ) 
...
; PROCEDURE doneg ( t : T ; stack_position := 0 ) 
  ...
  ; BEGIN 
    ...
    ; WITH stack0 = pos ( t , stack_position , "doneg" ) 
      , stop0 = t . vstack [ stack0 ] 
      DO 
        IF stop0 . loc = OLoc . imm 
        THEN 
          IF NOT TIntN . Negate ( stop0 . imm , neg ) 
          THEN 
...
; PROCEDURE doabs ( t : T ) 
  ...
  ; BEGIN 
      unlock ( t ) 
    ; WITH stack0 = pos ( t , 0 , "doabs" ) , stop0 = t . vstack [ stack0 ] 
      DO 
        IF stop0 . loc = OLoc . imm 
        THEN 
          IF NOT TIntN . Abs ( stop0 . imm , stop0 . imm ) 
          THEN 
...
; PROCEDURE doshift ( t : T ; type : IType ; shiftType : ShiftType ) : BOOLEAN 
  ...
  ;   shiftCount : INTEGER 
  ;   typeBitSize 
      := TIntN . T { x := Target . Int { IntType [ type ] . size , 0 , .. } } 
  ; BEGIN 
    ...
    ; WITH stack0 = pos ( t , 0 , "doshift" ) 
      ...
      DO 
       ...
       <* ASSERT TypeIs64(type) = TypeIs64(stop1.optype) *> 
        ...
        IF stop0 . loc = OLoc . imm 
           AND TWordN . LT ( stop0 . imm , TIntN . SixtyFour ) 
        THEN 
          IF stop1 . loc = OLoc . imm 
          THEN 
            IF NOT TIntN . ToHostInteger ( stop0 . imm , shiftCount ) 
            THEN 
              Err 
                ( t , "doshift: unable to convert target integer to host integer" ) 
            END 
            ...
            ; CASE shiftType 
              ...
              | ShiftType . UnboundedPositiveIsLeft 
                =>  IF TIntN . GT ( stop0 . imm , MaximumShift [ type ] ) 
                    ...
                    END 
              | ShiftType . LeftAlreadyBounded , ShiftType . RightAlreadyBounded 
                =>  TWordN . And 
                      ( stop0 . imm , MaximumShift [ type ] , tShiftCount ) 
                  ; stop0 . imm := tShiftCount 
                  ; IF shiftType = ShiftType . LeftAlreadyBounded 
                    THEN 
                      t . cg . immOp ( Op . oSHL , stop1 , stop0 . imm ) 
                    ELSE 
    ...
    ; WITH stack0 = pos ( t , 0 , "dorotate" ) 
      ...
      DO 
        ...
        THEN 
          IF stop1 . loc = OLoc . imm 
          THEN 
            IF NOT TIntN . ToHostInteger ( stop0 . imm , rotateCount ) 
            THEN 
              Err 
                ( t , "dorotate: failed to convert rotateCount to host integer" ) 
            END 
          ...
          ELSE 
            ...
            THEN 
              ...
              IF is64 
              THEN (* needs work to be more efficient, but ok *) 
                RETURN FALSE 
              END 
 
            ; find ( t , stack1 , Force . anytemp ) 
 
            ; IF TIntN . GT ( stop0 . imm , TZero ) 
              THEN 
                TWordN . And 
                  ( stop0 . imm , BitCountMask [ type ] , stop0 . imm ) 
              ; t . cg . immOp ( Op . oROL , stop1 , stop0 . imm ) 
              ELSE 
                IF NOT TIntN . Negate ( stop0 . imm , stop0 . imm ) 
                THEN 
 ...
 *) 
  ...
  ; BEGIN 
    ...
    ; IF sign_extend AND t . vstack [ count ] . loc # OLoc . imm 
      THEN 
        Err ( t , "doextract: sign_extend requires constant offset/count" ) 
      END 
 
    ; IF t . vstack [ count ] . loc = OLoc . imm 
      THEN 
        IF NOT TIntN . ToHostInteger 
             ( t . vstack [ count ] . imm , count_value ) 
        THEN 
    ...
    ; 
      ...
      THEN 
        t . cg . immOp 
          ( Op . oSHL , t . vstack [ mask ] , t . vstack [ count ] . imm ) 
      ELSE 
        t . cg . unOp ( Op . oSHL , t . vstack [ mask ] ) 
      ; (* shift by ECX *) 
      END 
    ...
    ; 
      ...
      IF t . vstack [ offset ] . loc = OLoc . imm 
      THEN 
            (* Mask count to 31 or 63 -- should be redundant but is safe. *) 
        TWordN . And 
          ( t . vstack [ offset ] . imm 
    ...
    ; 
      ...
      THEN 
        t . cg . immOp 
          ( Op . oSHR , t . vstack [ value ] , t . vstack [ offset ] . imm ) 
      ELSE 
        t . cg . unOp ( Op . oSHR , t . vstack [ value ] ) 
      ; (* shift by ECX *) 
      END 
...
; PROCEDURE doextract_n 
    ( t : T ; type : IType ; sign_extend : BOOLEAN ; count : CARDINAL ) 
  = VAR andval : TIntN . T 
  ;   offset : INTEGER 
  ...
  ; BEGIN 
    ...
    ; WITH stack_offset = pos ( t , 0 , "extract_n" ) 
      ...
      , op_value = t . vstack [ stack_value ] 
      DO 
 
        IF sign_extend AND op_offset . loc # OLoc . imm 
        THEN 
          Err ( t , "doextract: sign_extend requires constant offset/count" ) 
        END 
 
      ; IF op_offset . loc = OLoc . imm 
        THEN 
          IF NOT TIntN . ToHostInteger ( op_offset . imm , offset ) 
          THEN 
      ...
      ; t . cg . unOp ( Op . oSHR , op_value ) 
      ; (* shift by ECX *) 
 
        IF count < typeBitSize 
        THEN 
...
; PROCEDURE doextract_mn 
    ( t : T ; type : IType ; sign_extend : BOOLEAN ; offset , count : CARDINAL ) 
  ...
  ; BEGIN 
    ...
    ; unlock ( t ) 
    ; WITH stack0 = pos ( t , 0 , "extract_mn" ) , stop0 = t . vstack [ stack0 ] 
      DO 
      ...
      ; IF sign_extend 
        THEN 
          find ( t , stack0 , Force . anyreg ) 
        ; IF ( offset + count ) < typeBitSize 
          THEN 
            IF NOT TIntN . FromHostInteger 
                 ( typeBitSize - ( offset + count ) 
            ...
            THEN 
              Err 
                ( t 
                , "doextract_mn: failed to convert " & Fmt . Int ( typeBitSize ) 
                  & " - (offset + count) to target integer" 
                ) 
            END 
        ...
        ; IF count < typeBitSize 
          THEN 
            IF NOT TIntN . FromHostInteger 
                 ( typeBitSize - count , Target . Integer . bytes , tint ) 
            THEN 
              Err 
                ( t 
                , "doextract_mn: failed to convert " & Fmt . Int ( typeBitSize ) 
                  & " - count to target integer" 
                ) 
            END 
        ...
        ; IF offset > 0 
          THEN 
            IF NOT TIntN . FromHostInteger 
                 ( offset , Target . Integer . bytes , tint ) 
            THEN 
 ...
 *) 
  ...
  ; BEGIN 
     ...
     *) 
      ...
      IF t . vstack [ count ] . loc = OLoc . imm 
         AND t . vstack [ offset ] . loc = OLoc . imm 
      THEN 
        IF NOT TIntN . ToHostInteger 
             ( t . vstack [ count ] . imm , count_value ) 
        THEN 
        ...
        END 
      ; IF NOT TIntN . ToHostInteger 
             ( t . vstack [ offset ] . imm , offset_value ) 
        THEN 
    ...
    ; IF t . vstack [ count ] . loc = OLoc . register 
      THEN 
        t . cg . unOp ( Op . oSHL , t . vstack [ mask ] ) 
      ; (* shift by ECX *) 
      ELSE 
    ...
    ; IF t . vstack [ offset ] . loc = OLoc . register 
      THEN 
        t . cg . unOp ( Op . oSHL , t . vstack [ mask ] ) 
      ; (* shift by ECX *) 
        t . cg . unOp ( Op . oSHL , t . vstack [ from ] ) 
      ; (* shift by ECX *) 
      ELSE 
        t . cg . immOp 
          ( Op . oSHL , t . vstack [ mask ] , t . vstack [ offset ] . imm ) 
      ; t . cg . immOp 
          ( Op . oSHL , t . vstack [ from ] , t . vstack [ offset ] . imm ) 
      END 
    ...
    ; WITH stack_from = pos ( t , 0 , "insert" ) 
      ...
      ; IF op_from . loc = OLoc . imm 
        ...
        ; IF offset # 0 
          THEN 
            IF NOT TIntN . FromHostInteger 
                 ( offset , Target . Integer . bytes , tint_m ) 
            THEN 
              Err 
                ( t , "doinsert_mn: unable to convert offset to target integer" ) 
            END 
      ...
      ; IF TWordN . NE ( mask , max ) 
        THEN 
          IF op_to . loc = OLoc . imm 
          THEN 
      ...
      ; IF op_to . loc = OLoc . imm 
        THEN 
          ...
          ELSE 
            swap ( t ) 
          ; IF op_from . loc # OLoc . imm 
               OR TIntN . NE ( op_from . imm , TZero ) 
            THEN 
              t . cg . binOp ( Op . oOR , op_to , op_from ) 
            END 
          END 
        ELSE 
          IF op_from . loc # OLoc . imm OR TIntN . NE ( op_from . imm , TZero ) 
          THEN 
            t . cg . binOp ( Op . oOR , op_to , op_from ) 
          END 
        END 
...
; PROCEDURE swap ( t : T ) 
  = VAR tmp : Operand 
  ; BEGIN 
      WITH stack0 = pos ( t , 0 , "swap" ) , stack1 = pos ( t , 1 , "swap" ) 
      DO 
      ...
      ; IF t . vstack [ stack0 ] . loc = OLoc . register 
        THEN 
          FOR i := 0 TO GetOperandSize ( t . vstack [ stack0 ] ) - 1 
          DO 
      ...
      ; IF t . vstack [ stack1 ] . loc = OLoc . register 
        THEN 
          FOR i := 0 TO GetOperandSize ( t . vstack [ stack1 ] ) - 1 
          DO 
      ...
      ; IF t . vstack [ stack0 ] . loc = OLoc . fstack 
           AND t . vstack [ stack1 ] . loc = OLoc . fstack 
        THEN 
          t . cg . fstack_swap ( ) 
        END 
      END 
...
; PROCEDURE doloophole ( t : T ; from , to : ZType ) 
  ...
  ; BEGIN 
      WITH stack0 = pos ( t , 0 , "doloophole" ) , stop0 = t . vstack [ stack0 ] 
      DO 
 
        IF fromFloat AND toFloat 
        THEN 
 
          (* no code is needed *) 
 
        ELSIF ( NOT fromFloat ) AND ( NOT toFloat ) 
        THEN 
          ...
          ; CASE stop0 . loc 
            ...
            | OLoc . mem , OLoc . imm , OLoc . register 
              =>  IF fromSize = 2 
                  THEN 
                    find ( t , stack0 , Force . anyreg ) 
                  ELSE 
        ...
        ; stop0 . mvar . var 
            := t . parent . declare_temp 
                 ( CG_Bytes [ to ] , CG_Align_bytes [ to ] , to , FALSE ) 
        ...
        <* ASSERT toFloat *> 
          IF stop0 . loc = OLoc . mem 
             AND CG_Bytes [ stop0 . mvar . mvar_type ] < 4 
          THEN 
            find ( t , stack0 , Force . anyreg ) 
          END 
 
        ; IF stop0 . loc = OLoc . register OR stop0 . loc = OLoc . imm 
          THEN 
    ...
    ; WITH stack0 = pos ( t , 0 , "doindex_address" ) 
      ...
      ; IF stop0 . loc = OLoc . imm 
        THEN 
          ...
          THEN 
            Err 
              ( t , "doindex_address: failed to convert size to target integer" ) 
          END 
        ; IF NOT TIntN . Multiply ( stop0 . imm , tsize , tint ) 
          THEN 
            Err ( t , "doindex_address: multiply overflowed" ) 
          END 
        ; stop0 . imm := tint 
        ELSE 
          IF stop0 . loc # OLoc . register AND shift >= 0 
          THEN 
            find ( t , stack0 , Force . anyreg ) 
          END 
        ; IF stop0 . loc = OLoc . mem AND shift < 0 
             AND CG_Bytes [ stop0 . mvar . mvar_type ] < 4 
          THEN 
            find ( t , stack0 , Force . anydword ) 
          END 
        ...
        ; IF shift < 0 
          ...
          ; IF stop0 . loc # OLoc . register 
            THEN 
              muldest := pickreg ( t ) 
            ; corrupt ( t , muldest , operandPart := 0 ) 
            ; t . cg . imulImm 
                ( t . cg . reg [ muldest ] , stop0 , size , imsize ) 
          ...
          THEN 
            IF NOT TIntN . FromHostInteger 
                 ( shift , Target . Integer . bytes , tshift ) 
            THEN 
              Err 
                ( t , "doindex_address: failed to convert size to target integer" ) 
            END 
...
; TYPE MaxMinRec = RECORD regreg , regmem , memreg : Cond ; END 
 
; TYPE MaxMinCond = ARRAY [ Type . Word32 .. Type . Reel ] OF MaxMinRec 
 
; CONST maxmincond 
    = ARRAY MaxMin OF MaxMinCond 
        { 
              (* MAX *) 
          MaxMinCond 
            { MaxMinRec { Cond . A , Cond . AE , Cond . BE } 
            ,                                             (* Word32 *) 
            ...
            ,                                             (* Int64  *) 
              MaxMinRec 
                { Cond . AE , Cond . AE , Cond . AE }     (* Reel   *) 
            } 
        , 
(* MIN *) 
          MaxMinCond 
            { MaxMinRec { Cond . B , Cond . BE , Cond . AE } 
            ,                                             (* Word32 *) 
  ...
  ; BEGIN 
      ...
      ; IF NOT ftop_inmem 
        THEN 
          t . cg . f_pushnew ( ) 
        ; 
          (* It thinks we have just discarded something 
      ...
      ; 
        ...
        ; lab := t . cg . reserve_labels ( 1 , TRUE ) 
        ; IF destop . loc = OLoc . register OR srcop . loc = OLoc . imm 
          THEN 
            IF srcop . loc = OLoc . register OR srcop . loc = OLoc . imm 
            THEN 
...
; PROCEDURE fltoint ( t : T ; mode : FlToInt ; <*UNUSED*> type : Type ) 
  ...
  ; BEGIN 
    ...
    ; IF TIntN . NE ( t . rmode [ mode ] , TZero ) 
      THEN 
        t . cg . immOp 
          ( Op . oOR , t . cg . reg [ statreg ] , t . rmode [ mode ] ) 
      END 
    ...
    ; t . cg . memFOp 
        ( FOp . fISTP , t . vstack [ pos ( t , 0 , "fltoint" ) ] . mvar ) 
...
; PROCEDURE inttoflt ( t : T ) 
  = BEGIN 
      WITH stack0 = pos ( t , 0 , "inttoflt" ) , stop0 = t . vstack [ stack0 ] 
      DO 
        IF stop0 . loc = OLoc . mem 
           AND CG_Bytes [ stop0 . mvar . mvar_type ] < 4 
        THEN 
...
; PROCEDURE newdest ( t : T ; READONLY op : Operand ) 
  = BEGIN 
      IF op . loc = OLoc . register 
      THEN 
        FOR i := 0 TO GetTypeSize ( op . optype ) - 1 
        DO 
          WITH z = t . reguse [ op . reg [ i ] ] 
          DO 
...
; PROCEDURE expand_stack ( t : T ) 
  = BEGIN 
      ...
      THEN 
        WITH newarr = NEW ( REF ARRAY OF Operand , t . vstacklimit * 2 ) 
        DO 
          FOR i := 0 TO ( t . vstacklimit - 1 ) 
          DO 
            newarr [ i ] := t . vstack [ i ] 
          END 
...
; PROCEDURE discard ( t : T ; depth : CARDINAL ) 
  = BEGIN 
      ...
      END 
    ; FOR i := t . stacktop - depth TO t . stacktop - 1 
      DO 
        WITH stackp = t . vstack [ i ] 
        DO 
          CASE stackp . loc 
          OF OLoc . mem 
            =>  IF stackp . mvar . var . stack_temp 
                THEN 
                  t . parent . free_temp ( stackp . mvar . var ) 
                END 
          | OLoc . register 
            =>  FOR j := 0 TO GetOperandSize ( stackp ) - 1 
                DO 
...
; PROCEDURE set_current_proc ( t : T ; LLProc : LLProcTyp ) 
  = BEGIN 
      t . current_proc := LLProc 
    END set_current_proc 
 
; PROCEDURE New ( parent : LLTypes . CodeGenTyp ; cg : LLCode . T ; debug : BOOLEAN ) : T 
...
; CONST OLocName = ARRAY OLoc OF TEXT { "MEM" , "REG" , "FSTACK" , "IMM" } 
 
; PROCEDURE Debug ( t : T ; tag : TEXT ; wr : LLWr . T ) 
  = VAR tos := t . stacktop - 1 
  ; BEGIN 
      IF NOT t . debug 
      THEN 
        RETURN 
...
; PROCEDURE DebugOp ( READONLY op : Operand ; wr : LLWr . T ) 
  = BEGIN 
      wr . OutT ( OLocName [ op . loc ] ) 
    ; wr . OutT ( "  mvar: " ) 
    ; DebugMVar ( op . mvar , wr ) 
    ; wr . OutT ( "  reg: " ) 
    ; wr . OutT ( RegName [ op . reg [ 0 ] ] ) 
    ; wr . OutT ( "  imm: " ) 
    ; wr . OutT ( TIntN . ToText ( op . imm ) ) 
    ; wr . OutT ( "  stackp: " ) 
    ; wr . OutI ( op . stackp ) 
    ; IF ( op . opcode ) 
      THEN 
...
; PROCEDURE DebugReg ( READONLY r : Register ; wr : LLWr . T ) 
  = BEGIN 
      IF r . stackp # - 1 
      THEN 
        wr . OutT ( "  stackp: " ) 
      ; wr . OutI ( r . stackp ) 
      END 
    ; IF r . last_store # NoStore 
      THEN 
        wr . OutT ( "  mvar: " ) 
      ; DebugMVar ( r . last_store , wr ) 
      END 
    ; IF ( NOT TIntN . EQ ( r . last_imm , TZero ) ) 
      THEN 
        wr . OutT ( "  imm: " ) 
      ; wr . OutT ( TIntN . ToText ( r . last_imm ) ) 
      END 
    ; IF ( NOT TIntN . EQ ( r . lowbound , TIntN . TargetIntegerMin ) ) 
      THEN 
        wr . OutT ( "  lo: " ) 
      ; wr . OutT ( TIntN . ToText ( r . lowbound ) ) 
      END 
    ; IF ( NOT TIntN . EQ ( r . upbound , TIntN . TargetIntegerMax ) ) 
      THEN 
        wr . OutT ( "  hi: " ) 
      ; wr . OutT ( TIntN . ToText ( r . upbound ) ) 
      END 
    ; IF ( r . imm # FALSE ) 
      THEN 
        wr . OutT ( "  IMMED" ) 
...
; PROCEDURE DebugMVar ( READONLY v : MVar ; wr : LLWr . T ) 
  = BEGIN 
      wr . OutT ( "{ " ) 
    ; wr . VName ( v . var ) 
    ; IF ( v . mvar_offset # 0 ) 
      THEN 
        wr . OutT ( "  offset: " ) 
      ; wr . OutI ( v . mvar_offset ) 
      END 
    ; wr . OutT ( "  type: " ) 
    ; wr . TName ( v . mvar_type ) 
    ; wr . OutT ( " }" ) 
    END DebugMVar 
